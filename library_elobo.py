...
# Qk= dictionry of the co-factor matrix for each block 
# yk= dictionry of the observations for each block 
# dim= dictionary storing the dimension of each block 
# first_raw = dictionary storing the the raw from which each block starts
# n= number of parameters
...
def elobo(A,Q,y, d, alfa, M,n):
    ...
    #inizialization of the variale I'll use for the loop for outlier detection
    x_mk={}
    s2_mk = {}
    F_ratio= {}
    Fk_lim= {}  
...
def classic_lobo(A, Q, y, d, alfa, M, n,):
    ...
    #cicling to compute LS for each block and statistics
    x_mk={}
    s2_mk = {}
    F_ratio={}
    for i in dim:
        #creating temporal copies of the original matrices ans each step removing the corresponding block
        A_mk = copy(Ak,i)
        Q_mk = copy(Qk,i)
        y_mk = copy(yk,i)
        dim_mk = copy(dim,i)
        ...
        #conputing the statistic for the test
